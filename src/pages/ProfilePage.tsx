...
const allergenOptions = ["Milk", "Egg", "Fish", "Shellfish", "Tree nuts", "Peanuts", "Wheat", "Soy", "Sesame"];
...
export function ProfilePage({ profile, onSave, onExport, onDelete }: Props) {
  ...
  const toggleListValue = (field: "equipment" | "cuisines" | "allergens", value: string) => {
    ...
    <main className="wrap profile-page">
      ...
      <div className="profile-layout">
        <section className="settings-card">
          ...
          <fieldset><legend>Known food allergens</legend><div className="preference-grid">{allergenOptions.map((item) => <label key={item}><input type="checkbox" checked={draft.allergens.includes(item)} onChange={() => toggleListValue("allergens", item)} />{item}</label>)}</div><small>These choices hide matching recipes. Always verify labels and cross-contact risk.</small></fieldset>
        ...
        <aside className="profile-sidebar">
          ...
          <section className="settings-card"><h2>Known allergens</h2>{draft.allergens.length ? <ul>{draft.allergens.map((allergen) => <li key={allergen}>{allergen}</li>)}</ul> : <p>None known</p>}<small>Save changes to apply them across discovery.</small></section>
